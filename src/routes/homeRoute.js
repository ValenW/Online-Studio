...
.get(function(req, res, next) {
	...
	}).populate('music_list').exec(function(error, tag0) {
		if (err) {
			console.log('Error in finding tag0.');
		} else {
			// sort musics of tag0 , get newest musics and hotest music.
			tag0_newest_music = tag0.music_list.sort(newest_cmp).slice(0, 6);
			tag0_hotest_music = tag0.music_list.sort(hotest_cmp).slice(0, 10);

			// finding tag1 begin
			Tag.findOne({
				tag_name: tag_name_list[1]
			}).populate('music_list').exec(function(error, tag1) {
				if (err) {
					console.log('Error in finding tag1.');
				} else {
					// sort musics of tag0 , get newest musics and hotest music.
					tag1_newest_music = tag1.music_list.sort(newest_cmp).slice(0, 6);
					tag1_hotest_music = tag1.music_list.sort(hotest_cmp).slice(0, 10);

					// finding tag2 begin
					Tag.findOne({
						tag_name: tag_name_list[2]
					}).populate('music_list').exec(function(error, tag2) {
						if (err) {
							console.log('Error in finding tag2.');
						} else {
							// sort musics of tag0 , get newest musics and hotest music.
							tag2_newest_music = tag2.music_list.sort(newest_cmp).slice(0, 6);
							tag2_hotest_music = tag2.music_list.sort(hotest_cmp).slice(0, 10);

							// finding tag3 begin
							Tag.findOne({
								tag_name: tag_name_list[3]
							}).populate('music_list').exec(function(error, tag3) {
								if (err) {
									console.log('Error in finding tag3.');
								} else {
									// sort musics of tag0 , get newest musics and hotest music.
									tag3_newest_music = tag3.music_list.sort(newest_cmp).slice(0, 6);
									tag3_hotest_music = tag3.music_list.sort(hotest_cmp).slice(0, 10);
									...
									res.render('home', {
										...
										tags: {
											tag0: {
												...
												hotest_music: tog0_hotest_music
											},
											tag1: {
												tag: tag1
